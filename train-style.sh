...
TRAIN_FOLDER=1_style
MODEL_NAME="JuMingArt"
max_train_steps=1500
num_ckpts=5

image_count=$(ls ./train_image/"$TRAIN_FOLDER"/*.jpg 2>/dev/null | wc -l)
if [ "$image_count" -eq 0 ]; then
    echo "Gen style."
    python module/gen_style.py "$MODEL_NAME"
fi
image_count=$(ls ./train_image/"$TRAIN_FOLDER"/*.jpg 2>/dev/null | wc -l)
...
bucket_reso_steps=$(jq -r '.bucket_reso_steps' $CONFIG_PATH)
...
random_crop=$(jq -r '.random_crop' $CONFIG_PATH)
